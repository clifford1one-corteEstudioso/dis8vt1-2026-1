# estructura y temas

1. PRELIMINARES
    1.1. Abstract
    1.2. Agradecimiento 
    1.3. Motivación personal: STOP HAVING POTENTIAL: 
2. INTRODUCCIÓN Y MARCO TEÓRICO
    2.1. Introducción
    2.2. Problemática
	    2.2.1. Arista 1
        2.2.2. Arista 2
        2.2.2. Arista 3
    2.3. Marco Teórico
        2.3.1 Biología
        2.3.2 Ética
        2.3.3 Diseño UX/UI



## problemática general

El diseño UX nació para hacer la tecnología más humana (Usabilidad), para orientar la tecnología hacia el usuarie. Sin embargo, en la actualidad, estas herramientas se han refinado para eliminar toda fricción y explotar vulnerabilidades biológicas, generando un choque entre la eficiencia técnica y la autonomía de les usuaries.

### problemáticas específicas

Biología: No conocemos las consecuencias de la exposición constante a redes sociales y picos de dopamina.

Diseño UX: La falta de regulación frente a prácticas predatorias por parte de empresas de medios digitales.

Ética: El dilema ético que representa el estudio del cerebro, con fines de lucro por parte de las empresas.

## clase hoy

- Marshall McLuhan: el filósofo que predijo las redes sociales. "El medio es el mensaje" - la comunicacion como sujeto de estudio

- investigar la web 2.0. Cuando el internet se volvió comunidad

- a medidad que se fue reduciendo la longitud del mensaje, antes eran diarios, depues columnas, depues blogs, despues tweets, dps notificaciones. Y nosotros comnezamos a personalizar el medio para que tenga mas info. Ejemplo distintos osnido de notif, segun longitud sabes si es llamada o mensaje, etc.

- alvin toeffler



